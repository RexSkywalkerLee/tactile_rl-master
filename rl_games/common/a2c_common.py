...
class A2CBase(BaseAlgorithm):
	def __init__(self, base_name, params):
		...
		if self.env_info is None:
			...
			#
			# env_config = {""}
			# print("ENVNAME", self.env_name) # rlgpu
			self.vec_env = vecenv.create_vec_env(self.env_name, self.num_actors, **self.env_config)
			self.env_info = self.vec_env.get_env_info()
			#print("ENTER A", self.env_info)
		else:
			self.vec_env = config.get('vec_env', None)
			#print("ENTER B", self.env_info)

		# generate test override args.
		# test_env_config = {'override': {'numTestEnvs': 16, 'env.m_low': 0.1, 'env.m_up': 0.3}}
		# self.test_vec_env = vecenv.create_vec_env(self.env_name, self.num_actors, **test_env_config)
	...
	def play_steps(self):
		...
		for n in range(self.horizon_length):
			...
			# THIS IS DANGEROUS. res_dict['actions'] = torch.clamp(res_dict['actions'], -1.0, 1.0)
		...
		for n in range(self.horizon_length):
			...
			if n % self.seq_len == 0:
				for s, mb_s in zip(self.rnn_states, mb_rnn_states):
					...
					# print(n//self.seq_len, mb_s.shape, mb_rnn_states[0].shape)
			...
			#print("RNN_STATES", self.rnn_states[0].shape) #[1, NUM_ENVS, HIDDEN_DIM]
			self.experience_buffer.update_data('obses', n, self.obs['obs'])
...
class ContinuousA2CBase(A2CBase):
	...
	def prepare_dataset(self, batch_dict):
		...
		# print("DATASET, OR", obses.shape, rnn_states[0].shape)
