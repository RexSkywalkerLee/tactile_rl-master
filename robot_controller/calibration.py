...
class AllegroArm:
	def __init__(self, p_gain=10.0, d_gain=0.1, dt=0.0167, rl_device='cuda:0', sim_device='cuda:0',
		...
		self.robot_asset_files_dict = {
			"normal": "urdf/xarm6/xarm6_allegro_right_fsr_2023_thin.urdf",
			"thick": "urdf/xarm6/xarm6_allegro_right_fsr_2023_thin.urdf",
			"large":  "urdf/xarm6/xarm6_allegro_right_fsr_2023_thin.urdf"
		}
	...
	def set_sim_params(self, dt):
		...
		sim_params.use_gpu_pipeline = False 	# config_sim["use_gpu_pipeline"]
		...
		setattr(sim_params.physx, "use_gpu", False)
...
if __name__ == '__main__':
	...
	all_tests = [(init_qpos, ImpulseFunc(0.0, horizon=6000)),
	...
	for test_idx, test in enumerate(all_tests):
		...
		env.goto_qpos(test_init_qpos, 6000)

		print("Start Testing")
		results = env.goto_qpos_seq(target_qpos, 6000) # 2s. Numpy array: [T, JOINT_QPOS, JOINT_QPOS]
		print("Done")
		...
		for i in range(16):
			...
			plt.title(finger_mapping_dict[i + 6])
