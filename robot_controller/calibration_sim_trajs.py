...
class AllegroArm:
	def __init__(self, p_gain=10.0, d_gain=0.1, dt=0.0167, rl_device='cuda:0', sim_device='cuda:0',
		...
		self.use_relative_control = True 		# self.cfg["env"]["useRelativeControl"]
...
if __name__ == '__main__':
	...
	all_trajectories = np.load('./qpos_trajectory.npy', allow_pickle=True)
	all_names = ["joint_0.0","joint_1.0","joint_2.0","joint_3.0",
	...
	# plt.ylim(-2, 2)
	#
	for test_idx, test in enumerate(all_trajectories):
		target, qpos = test['target'], test['qpos']
		print(target.shape[0])
		length = target.shape[0]
		print("LENGTH", length)
		for i in range(16):
			plt.subplot(4, 4, i + 1)
			plt.plot(np.linspace(0, length - 1, length) * 0.016667, qpos[:, i], c='b', label="tracking")
			plt.plot(np.linspace(0, length - 1, length) * 0.016667, target[:, i], c='r', label="target")
